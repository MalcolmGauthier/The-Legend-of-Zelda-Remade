    ...
    public static class Link
    {
        ...
        public static void Tick()
        {
            ...
            if (hp > nb_of_hearts[current_save_file])
                hp = nb_of_hearts[current_save_file];
            else if (hp < 0)
                hp = 0;
            
            counterpart.x = (short)(self.x + 8);
        ...
        {
            if (Sound.recorder_playing)
            {
                if (!Sound.RecorderPlaying())
                {
                    can_move = true;
        ...
        { // returns true if in animation for smthn related to the B button
            if (IsPressed(Buttons.B) && !using_item)
            {
                switch (Menu.current_B_item)
                {
                    ...
                    case 0x24:
                        ...
                        if (OverworldCode.current_screen != 66)
                        {
                            ...
                            {
                                new TornadoSprite(0, (short)y);
                                Menu.tornado_out = true;
                            }
                        }
                        else
                        {
                            if (OverworldCode.room_66_timer == 255)
                                OverworldCode.room_66_timer = 0;
                        }
                        break;
                    case 0x26:
                    ...
                    case 0x28:
                        if (!Menu.arrow_out && rupy_count[current_save_file] > 0)
                        {
                            ...
                            Menu.arrow_out = true;
                            Attack();
                            short[] pos = SetItemPos(facing_direction == Direction.LEFT || facing_direction == Direction.RIGHT);
                    ...
                    case 0x40:
                        ...
                        {
                            Menu.hud_B_item.tile_index = 0x4c;
                ...
                if (Menu.current_B_item != 0 && Menu.current_B_item != 0x24)
                    using_item = true;
                return true;
            }
            else if (using_item)
        ...
        static short[] SetItemPos(bool double_wide)
        {
        ...
        static void ChangePos(bool is_x, bool is_positive)
        {
            ...
            if (stair_speed)
            {
                ...
                change = (sbyte)((Program.gTimer & 1) + 1);
        ...
        static void GotoMod8()
        {
            ...
            if (facing_direction == Direction.LEFT || facing_direction == Direction.RIGHT)
            {
                byte diff_from_mod8 = (byte)(self.x & 7);
                if (diff_from_mod8 > 3)
                {
                    self.x += (short)(8 - diff_from_mod8);
                }
                else
                {
            ...
            {
                byte diff_from_mod8 = (byte)(self.y & 7);
                if (diff_from_mod8 > 3)
                {
                    self.y += (short)(8 - diff_from_mod8);
                }
                else
                {
        ...
        public static void SetBGState(bool apply_to_self, bool apply_to_counterpart, bool bg_mod_activated)
        {
            if (apply_to_self)
                self.background = bg_mod_activated;
            if (apply_to_counterpart)
        ...
        static void CheckCollision(short x_add, short y_add, ref bool collision) // create duplicate for dungeon
        {
            int metatile_index = ((self.y + y_add) & 0xFFF0) + ((self.x + x_add) >> 4) - 64;
        ...
        public static void TakeDamage(float damage)
        {
            ...
            hp -= damage;
            iframes_timer = 48;
            if (((int)knockback_direction < 2 && x % 8 == 0) || ((int)knockback_direction >= 2 && y % 8 == 0)) // if aligned on grid
                knockback_timer = 8;
        ...
        public static bool AddRupees(int change, bool perform_addition_automatically = true)
        { // returns false if underflow or overflow, true if transaction went smoothly
            if (rupy_count[current_save_file] + change < 0)
            {
                if (perform_addition_automatically)
                    rupy_count[current_save_file] = 0;
                return false;
            }
            else if (rupy_count[current_save_file] + change > 255)
            {
                if (perform_addition_automatically)
                    rupy_count[current_save_file] = 255;
                return false;
            }
            else
            {
                if (perform_addition_automatically)
                    rupy_count[current_save_file] += (byte)change;
                return true;
            }
        }
