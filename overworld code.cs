...
namespace The_Legend_of_Zelda
{
    internal static class OverworldCode
    {
        ...
        public static ushort warp_animation_timer = 0;
        ...
        static void Scroll()
        {
            ...
            void EndScroll()
            {
                ...
                SpawnEnemies();
                if (current_screen == 95 && !GetHeartContainerFlag(current_save_file, 12))
        ...
        public static byte ChangeRecorderDestination(bool change_the_value)
        {
            for (byte i = 0; i < 8; i++)
            {
                if (change_the_value)
                {
                    recorder_destination += add_to_recorder_counter;

                    if (recorder_destination < 0)
                        recorder_destination = 7;
                    else if (recorder_destination > 7)
                        recorder_destination = 0;
                }

                if (GetTriforceFlag(current_save_file, i))
                {
        ...
        static void CheckForWarp()
        {
            ...
            else if (black_square_stairs_flag)
            {
                BlackSquareLinkAnimation(true);
            }
            else if (stair_warp_flag)
            {
                if (current_screen == 128)
                {
                    if (Link.y < 200)
        ...
        public static void BlackSquareLinkAnimation(bool entering, bool immediate_exit = false)
        {
            ...
            if (warp_animation_timer > 64)
                Link.SetPos(new_y: Link.y - 1 - (Program.gTimer & 1));
        ...
        static void Level7EntranceAnimation()
        {
            ...
            switch (level_7_entrance_timer)
            {
                case 0:
                    ...
                    break;
                case 1:
                    Palettes.LoadPalette(3, 3, 0x11);
                    break;
                case 9:
                    Palettes.LoadPalette(3, 3, 0x22);
                    break;
                case 17:
                    Palettes.LoadPalette(3, 3, 0x21);
                    break;
                case 25:
                    Palettes.LoadPalette(3, 3, 0x31);
                    break;
                case 33:
                    Palettes.LoadPalette(3, 3, 0x32);
                    break;
                case 41:
                    Palettes.LoadPalette(3, 3, 0x33);
                    break;
                case 49:
                    Palettes.LoadPalette(3, 3, 0x35);
                    break;
                case 57:
                    Palettes.LoadPalette(3, 3, 0x34);
                    break;
                case 65:
                    Palettes.LoadPalette(3, 3, 0x36);
                    break;
                case 73:
                    Palettes.LoadPalette(3, 3, 0x37);
                    break;
                case 81:
                    for (int i = 0; i < 4; i++)
                        for (int j = 0; j < 6; j++)
                            meta_tiles[53 + 16 * i + j].tile_index = 1;
